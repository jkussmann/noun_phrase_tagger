"""
This module call the noun phrase taggers and returns
...
def get_similarity_score(string1, string2):
    """
	Return a similarity score of 2 strings
	"""
...
def features(tokens, index, history):
    ...
    tokens = (
        [("__START2__", "__START2__"), ("__START1__", "__START1__")]
        + list(tokens)
...
class ClassifierChunkParser(ChunkParserI):
    """"
	This is the class for the classifier chunk parser
	"""
    def __init__(self, chunked_sents, **kwargs):
        ...
        # Transform the triplets in pairs, make it compatible with the tagger
        # interface [((word, pos), chunk), ...]
        ...
        self.tagger = ClassifierBasedTagger(
            train=chunked_sents, feature_detector=features, **kwargs
        )
...
def train_chunker():
    """
	This functioni trains the chunker
	"""
...
def get_chunker():
	...
	"""
    if not os.path.isfile(MODEL_PATH + "chunk_model.pkl"):
        print("")
...
def get_spacy_nps(line):
    ...
    for noun_chunk in tagged_line.noun_chunks:
        noun_phrase_list.append(noun_chunk.text)
...
def get_noun_phrases(line):
    ...
    for item1 in stanford_nps:
        for item2 in spacy_nps:
            if (get_similarity_score(item1, item2) > 0.80) or (
                item1 in item2 or item2 in item1
            ):
                append_item = False
